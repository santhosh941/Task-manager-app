...
const Task = () => {
  ...
  useEffect(() => {
    if (mode === "update") {
      ...
      fetchData(config, { showSuccessToast: false }).then((data) => {
        setTask(data.task);
        setFormData({ taskname: data.task.taskname,description: data.task.description,priority:data.task.priority,
  ...
  const handleReset = e => {
    ...
    setFormData({
      ...
      description: task.description,priority:task.priority,dueDate: task.dueDate.split('T')[0]
    });
  }
          ...
          {loading ? (
            ...
            <>
              ...
              <div className='mb-4'>
               ...
               <input type='text'className='ml-4 border-4' name='taskname'id='taskname'value={formData.taskname} onChange={handleChange}required placeholder='Enter task name'/>
               {fieldError("taskname")}
               </div>
               <div className="mb-4">
              ...
              <div className="mb-4">
                <label htmlFor="dueDate">Set DueDate: </label>
                <input 
                  type="date" 
                  name="dueDate" 
                  id="dueDate" 
