...
exports.postTask = async (req, res) => {
  try {
    ...
    if (!taskname || !description || !priority || !dueDate) {
      return res.status(400).json({ status: false, msg: "All fields are reqiured" });
    }
    const dueDateObj = new Date(dueDate);
    const task = await Task.create({ user: req.user.id, taskname,description,priority,dueDate });
...
exports.updateTaskStatus = async (req, res) => {
  try {
    ...
    const updatedTask = await Task.findByIdAndUpdate(req.params.taskId, { status: newStatus }, { new: true });
